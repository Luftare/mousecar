...
export default class View {
  constructor({
    ...
    this.handleResize();
    this.fillScreen();
    this.images = this.loadImages(sources);
  }
  ...
  preDraw() {
    ...
    this.ctx.setTransform(
      ...
      0,
      this.camera.zoom,
      -(this.camera.zoom - 1) * this.canvas.width * this.camera.anchor.x,
      -(this.camera.zoom - 1) * this.canvas.height * this.camera.anchor.y
    );
    this.ctx.translate(
      this.canvas.width * this.camera.anchor.x,
      this.canvas.height * this.camera.anchor.y
    );
  ...
  loadImages(sources) {
    return sources.reduce((images, item) => {
